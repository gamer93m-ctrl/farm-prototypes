...
/* Правило 2 из макета: выбранная ячейка запирает остальные, пока её не
   обработают или не отожмут. Замок был слепым — не смотрел, участвует ли
   выбранная ячейка в том действии, которое идёт сейчас. Выделил пустую,
   взял ручку — и сбор запирался на ячейке, с которой собирать нечего:
   сетка не отдавала ничего вообще, пока выделение не снимешь.

   Поэтому спрашиваем замок под конкретное действие: он держит, только
   если выбранная ячейка — законная цель этого действия. Пустая ячейка
   при сборе к сбору отношения не имеет и никого не запирает. */
function lockFor(kind){
  if(selCell === null) return null;
  const c = state.cells[selCell];
  if(!c) return null;
  const fits = kind === 'seed' ? c.state === 'empty' : c.state === 'ready';
  return fits ? selCell : null;
}
...
function apply(x, y){
  ...
  // Во второй версии замок спрашиваем под действие, в первой оставляем
  // как было — онбординги должны вести себя ровно по-старому.
  const lock = V2 ? lockFor(drag.kind) : selCell;
  if(lock !== null && idx !== lock) return;
...
scroller.addEventListener('pointerdown', e => {
  ...
  // Зажали другую ячейку — старое выделение снимаем сразу, на нажатии.
  // Раньше оно доживало до клика, а если касание уходило в заметание или
  // клик проглатывался, то не снималось вовсе: на сетке висела синяя
  // ячейка, которая уже ничего не значила.
  if(V2 && selCell !== null && +cell.dataset.idx !== selCell) clearSel();
...
function startSweep(){
  ...
  lockScroll(true);
  clearHolding();
  if(V2) hideTip();      // пошло заметание — открытый тултип тут только мешает
...
function sweepAt(x, y){
  ...
  // правило 2 работает и при заметании: выделил готовую ячейку — сбор
  // идёт с неё, остальные заперты, пока её не соберут или не отожмут.
  // Раньше замок знал только путь перетаскивания, поэтому выделение
  // при сборе рукой и удержанием висело зря и ни на что не влияло.
  if(V2){
    const lock = lockFor('collect');
    if(lock !== null && i !== lock) return;
  }
...
function endDirect(){
  ...
  /* В удержании быстрый тап больше не забирает молча. Сбор тут —
     осознанное действие, и одиночный тап не должен его подменять:
     он выделяет ячейку и открывает тултип с кнопкой «Собрать», ровно
     как в версии A. Выделенная ячейка дальше и задаёт, откуда пойдёт
     сбор рукой или удержанием (правило 2).

     Клик не проглатываем — им и займётся обработчик тапа ниже по файлу:
     он выберет ячейку и покажет тултип. Двойной тап там же остаётся
     быстрым сбором. */
  const holdTip = V2 && cfg.collect === 'direct' && cfg.direct === 'hold';
  ...
  } else if(tap && !holdTip && state.cells[dc.idx].state === 'ready'){
    collect(dc.idx);
    swallowClick = true;
  }
  lockScroll(false);
